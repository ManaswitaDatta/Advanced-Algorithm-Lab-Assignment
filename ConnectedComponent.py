...
def printConectedComponent(graph):
    ...
    visited = [False for i in range(graph.num_of_v)]
    for u in range(graph.num_of_v):
        if not visited[u]:
            v_rep = find(subsets, u)
            subsets[v_rep].list.printList(visited)
